...
### Sending Email
The SPA Template uses the awesome [Postal](http://aboutcode.net/postal) to send emails but it is abstracted from the rest of the code with the `IMailer`interface, if you want use something else other than Postal, just implement the interface with your implementation. The email templates are located under the `Views/Emails` folder. The sender email address is specified in `sender.email` key of `appSettings` section in the `web.config` file. Also note that it is currently configured to save the emails in the `c:\temp` folder instead of sending the actual mail, make sure you change the setting before going live.
